...
## [2026-04-24] lint-fix | All 5 findings resolved
- Report: [[lint-report-2026-04-24]] (now `status: mature`, Resolution Pass appended)
- **Actionable fixes (2):**
  - HIGH #2 `missing-sources-footer` — appended `## Sources` footer to 13 entity pages ([[Aidan N. Gomez]], [[Ashish Vaswani]], [[Google Brain]], [[Google Research]], [[Illia Polosukhin]], [[Jakob Uszkoreit]], [[Llion Jones]], [[Niki Parmar]], [[Noam Shazeer]], [[tensor2tensor]], [[University of Toronto]], [[WMT 2014]], [[Łukasz Kaiser]]) each citing `[[Attention Is All You Need]]`; renamed `## Raw Source` → `## Sources` on the source page itself.
  - LOW #1 `dead-links` — stripped `.md` suffix from `[[index.md]] → [[index]]`, `[[hot.md]] → [[hot]]`, `[[overview.md]] → [[overview]]` in the 2026-04-21 and 2026-04-22 entries of this log.
- **Verified stale at re-check (no edits needed, 3):**
  - HIGH #1 `source-status` — `raw/manifest.yaml` already carries 3 active entries for the Müller & Guido book (Chapter 4 markdown, Chapter 3 PDF, MinerU extraction).
  - HIGH #3 `dead-links` `[[ByteNet]]` — no such wikilink exists anywhere in the wiki; 4 plain-text mentions do not count.
  - LOW #2 `empty-sections` — all 5 flagged headings (`Data Preprocessing`, `Feature Engineering`, `Feature Selection`, `Unsupervised Learning`) already populated via the 2026-04-21 / 2026-04-22 ingests; scanner treated `###` subsections as non-content.
- Side-effect: [[trust-report]] refreshed — aggregate unchanged (0 low, 48 medium, 25 high) because no reliability signal (contradictions / supersessions / gaps / volatility) moved; only the `updated:` date bumped to 2026-04-24.
- Post-pass state: **0 outstanding lint findings.**
...
- LOW findings: 3 `.md`-suffixed wikilinks in log.md (`[[index]]`, `[[hot]]`, `[[overview]]`); 5 empty content-page section bodies (topic pages' `## Current Understanding`)
